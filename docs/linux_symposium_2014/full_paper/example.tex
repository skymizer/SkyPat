...
Furthermore, with the help of perf\_event of Linux kernel, SkyPat can provide precise timer with cycle-level accuracy .
...
To get performance of the region of code, SkyPat uses macro to assemble ``perf\_event'' interface into the test-case.
SkyPat uses ``perf\_event'' interface to register events to be monitored when program initials.
When program executes at the beginning of the region of code to be monitored, SkyPat will send a system call to kernel to enable the counter to get process execution time.
When program executes in the end of the region of code, SkyPat will send a system call to kernel to disable the counter to get process execution time.
SkyPat will get the difference of time between begin and end to calculate the execution time in the region of code.
Because SkyPat uses ``perf\_event'' rather than ``gettimeofday'' to get execution time, SkyPat can eliminate the effect from scheduling between processes.
By this way, SkyPat can provide user precise timer information of the region of code which can be assigned by programmer.

\section{Evaluation}

\section{Conclusion}
By integrating unit-test framework and performance evaluation tool, user can get accuracy and performance number by writing test-cases.
