...
When a program executes EXPECT assertion, although its condition fails, SkyPat displays ``error: failed to expect'' and keeps execution.
On the other side, when a program executes the last ASSERT assertion whose condtion is fall, because it is a fatal assertion, the program stops immediately and displays ``fatal: failed to assert''.
With the helps of these assertions, user can verify correctness of their program.
