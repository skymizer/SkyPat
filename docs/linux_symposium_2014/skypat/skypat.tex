...
From compiler developers' view, correctness and performance evaluation are two grand challenges for engineering.
Compiler optimizations have no guarantee of performance improvement, and the worst is: sometimes an optimization not only degrades performance but also introduces defeats in a program.
Compiler developers need a tool to verify correctness and performance of each compiler optimizations to make sure whether compiler optimization works or not.
As a result, SkyPat is developed for this kind of demand.
By integrating unit-test and performance evaluation tool, user can use SkyPat to evaluate correctness and performance by writing test-cases.
...
Meanwhile, \textit{perf} \cite{perf-tools} is well-known performance evaluation toolkit in Linux. 
...
Some tools focus on performance evaluation, such as Oprofile\cite{oprofile} and \textit{perf}. 
