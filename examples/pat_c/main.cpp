...
PAT_C(MyCase, test_turret_is_turn_by_default)
{
...
PAT_C(MyCase, test_turret_can_be_turn_off)
{
    Turret * turret = this->getTurret();
    turret->turnOff();
...
// test the turn on turn of functions of turret
PAT_C(MyCase, test_turret_can_fire_iff_turn_on)
{
    Turret * turret = this->getTurret();
    turret->turnOff();
...
// Performance Test
PAT_C(MyCase, performance_turret_fire) 
{
